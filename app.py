# app.py - PULSE Live Mumbai Navigator Engine with Integrated About Page
...
from about_pulse import render_about_page  # About PULSE Module
...
def main_app():
    ...
    # 📌 SIDEBAR NAVIGATION
    st.sidebar.markdown("---")
    app_view = st.sidebar.radio("📌 Navigation:", ["Live Transit Navigator 🚆", "About PULSE ℹ️"])
    st.sidebar.markdown("---")
    ...
    # 👑 RENDER ABOUT PULSE PAGE IF SELECTED
    if app_view == "About PULSE ℹ️":
        render_about_page()
        return  # Main tracking page render roko jab About section open ho
    ...
    if not st.session_state.tracking:
        ...
        if st.button(t["start"]):
            if source == destination:
                ...
                # 1. Log Journey to SQLite Database
                ...
                # 2. Log Journey Event to Administrative Master Audit CSV
                log_journey_event(st.session_state.username, selected_line, source, destination)
    ...
    # Phase 2: Live Tracking
        ...
        if st.session_state.tracking:
            ...
            with map_col:
                ...
                st_folium(m, width="stretch", height=360)
            ...
            with st.expander("🚨 Guardian Mode (Online Women's Safety)"):
                ...
                if st.button("🔴 TRIGGER ONLINE SOS ALERT", width="stretch"):
