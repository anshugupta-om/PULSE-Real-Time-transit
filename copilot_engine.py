...
def render_safety_copilot(source, destination, overall_density, live_weather_status):
    ...
    with st.expander("🎮 Humanoid Robot Pose & Expression Controls"):
        ...
        if p_col1.button("👋 Wave & Happy Pose", use_container_width=True):
            st.session_state.aura_mood_override = "HAPPY"
            st.rerun()
        if p_col2.button("🧐 Focused Alert Pose", use_container_width=True):
            st.session_state.aura_mood_override = "ALERT"
            st.rerun()
        if p_col3.button("😰 Worried Gesture Pose", use_container_width=True):
            st.session_state.aura_mood_override = "WORRIED"
            st.rerun()
        if p_col4.button("🔄 Auto Telemetry Mode", use_container_width=True):
